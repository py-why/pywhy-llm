...
class IdentificationSuggester(IdentifierProtocol):
    def suggest_estimand(
        ...
        if backdoor == None:
            backdoor = self.suggest_backdoor(
                confounders=confounders, treatment=treatment, outcome=outcome, llm=llm
            )
        if len(backdoor) > 0:
            estimands_dict["backdoor"] = construct_backdoor_estimand(
        ...
        if frontdoor == None:
            frontdoor = self.suggest_frontdoor(
                confounders=confounders, treatment=treatment, outcome=outcome, llm=llm
            )
        if len(frontdoor) > 0:
            estimands_dict["frontdoor"] = construct_frontdoor_estimand(
    ...
    def suggest_backdoor(
        self, confounders: List[str], treatment: str, outcome: str, llm: guidance.llm
    ):
            ...
            if confounder != treatment and confounder != outcome:
                ...
                while not success:
                    try:
                        ...
                        z = re.findall(
                            r"<answer>(.*?)</answer>", output["backdoor_set"], re.DOTALL
                        )
            ...
            if confounder != treatment and confounder != outcome:
                ...
                while not success:
                    try:
                        ...
                        z = re.findall(
                            r"<answer>(.*?)</answer>", output["iv_set"], re.DOTALL
                        )
